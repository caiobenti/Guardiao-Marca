...
const AVAILABLE_VARS = [
  '{{canal}}', '{{formato}}', '{{estilo}}', '{{objetivo}}', '{{tema}}',
  '{{persona_nome}}', '{{persona_descricao}}', '{{persona_dores}}', '{{persona_valor}}',
  '{{voz_tom}}', '{{voz_personalidade}}', '{{voz_linguagem}}', '{{voz_arquetipo}}',
  '{{voz_keywords}}', '{{voz_avoid}}',
  '{{dna_cores}}', '{{dna_tipografia}}', '{{dna_estilo_imagem}}',
]
...
export default function ParametroIAClient({ userId, userCode, initialConfig }: Props) {
  const [systemPrompt, setSystemPrompt] = useState(initialConfig?.system_prompt ?? DEFAULT_SYSTEM_PROMPT)
  const [userTemplate, setUserTemplate] = useState(initialConfig?.user_template ?? DEFAULT_USER_TEMPLATE)
  const [model, setModel] = useState(initialConfig?.model ?? 'llama-3.3-70b-versatile')
  ...
  async function handleSave() {
    ...
    let error
    if (existing?.id) {
      ({ error } = await supabase.from('DB4 - ia_config').update(payload).eq('id', existing.id))
    } else {
    ...
    <div className="max-w-3xl mx-auto px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-stone-400 mt-0.5">Configure o comportamento da inteligência artificial</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          {saveMsg && (
            <span className={`text-xs font-medium ${saveMsg.startsWith('Erro') ? 'text-red-600' : 'text-green-600'}`}>
              {saveMsg}
            </span>
          )}
          <button
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
          ...
          <p className="text-xs text-stone-400 mb-3">
            Instruções permanentes que definem o comportamento e identidade da IA. A IA segue estas regras em todas as gerações.
          </p>
          <textarea
            value={systemPrompt}
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
          ...
          <p className="text-xs text-stone-400 mb-3">
            Estrutura do pedido enviado à IA. Use <code className="bg-stone-100 px-1 rounded text-[#1a6b5a]">{'{{variavel}}'}</code> para inserir dados dinâmicos.
          </p>
          <textarea
            value={userTemplate}
          ...
          {/* Variables reference */}
          <div className="mt-4">
            <p className="text-xs text-stone-400 mb-2 font-medium">Variáveis disponíveis — clique para inserir:</p>
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-xs font-mono text-stone-600">{temperature.toFixed(1)}</span>
              </div>
              <input
                type="range"
            ...
            {/* Max Tokens */}
            <div>
              <label className="block text-xs text-stone-500 mb-2">Máximo de tokens</label>
              <input
                type="number"
              ...
              />
              <p className="text-xs text-stone-400 mt-1">Resposta mais longa = mais tokens. Máx Groq gratuito: 4000</p>
            </div>
          </div>
        </div>
      </div>
